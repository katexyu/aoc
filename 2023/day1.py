...
def solvea(inp):
    inp = inp.strip().split('\n')    
    ans = sum(numbersa(l) for l in inp)
    return ans
...
def solveb(inp):
    inp = inp.strip().split('\n')    
    ans = sum(numbersb(l) for l in inp)
    return ans 
...
def numbersb(line):
    ...
    for idx in range(ml):
        ...
        if c.isnumeric():
            numbers.append(c)
            continue
        candidates = [line[idx:min(idx+3,ml)], line[idx:min(idx+4,ml)], line[idx:min(idx+5,ml)]]
        for c in candidates:
            if c in mapping:
                numbers.append(str(mapping[c]))        
    return int(numbers[0]+numbers[-1])
