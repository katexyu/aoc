...
def solve1(inp):
    ...
    while queue:
        nxt, dst = queue.pop(0)
        neighbors = get_neighbors(mapping, nxt[0], nxt[1])
        for n in neighbors:
            if n not in visited:
                queue.append((n, dst+1))
                visited.add(n)
                max_so_far = max(max_so_far, dst+1) 
...
def get_neighbors(mapping, x, y):
    val = mapping[(x, y)]
    if val not in diffs:
        return []
    possible = diffs[val]
    n = []
    for dx, dy, _ in possible:
        xx, yy = x + dx, y + dy
        val = mapping.get((xx, yy), '.')
        if val in connects[(dx, dy)]:
            n.append((xx, yy))
    return n
    


def solve2(inp, start):
    inp = inp.strip().split('\n')
    mapping = to_grid_map(inp)
    ...
    while queue:
        ...
        neighbors = get_neighbors(mapping, nxt[0], nxt[1])
        for n in neighbors:
            if n not in nvisited:
                nvisited.add(n)
                queue.append((n))
                main_loop.add(n)
    
    # Now we know which tiles are part of the loop
    # Now need to determine how many tiles are inside loop 
    queue = [start]
    visited = set()
    visited_neighbors = set()
    # check outside corners first
    max_y = max(k[1] for k in mapping.keys())
    max_x = max(k[0] for k in mapping.keys())
    corners = [(0,0), (0, max_y), (max_x, 0), (max_x, max_y)]
    # corners are not in the loop by definition
    for cx, cy in corners:
        if (cx, cy) not in main_loop:
            adj = get_adjacent(mapping, main_loop, cx, cy)
            for ax, ay in adj:
                mapping[(ax, ay)] = 'O'
            visited_neighbors.update(adj)

    inside = 0
    while queue:
        nxt = queue.pop(0)
        anti_neighbors = get_anti_neighbors(mapping, main_loop, nxt[0], nxt[1])
        

        for x, y in anti_neighbors:
                
            adj = get_adjacent(mapping, main_loop, x, y)
            if (x,y) in mapping and (x,y) not in main_loop:
                adj.add((x,y))

            if (x, y) not in visited_neighbors:
                # adj is a connected component
                # just check one of them
                if check_inside(mapping, main_loop, x, y):
                    inside += len(adj)
                    for ax, ay in adj:
                        mapping[(ax, ay)] = '*'
                else:
...
def check_inside(mapping, main_loop, x, y):
    xx = x
    pipes = 0
    nxt = (xx-1, y)
    while nxt in mapping:
        nxtval = mapping.get(nxt)
        if nxt in main_loop and nxtval in ('|', '7', 'F'): 
            pipes += 1
        nxt = (nxt[0]-1, nxt[1])
...
def get_adjacent(mapping, main_loop, x, y):
    queue = [(x, y)]
    visited = set()
    ...
    while queue:
        ...
        for dx, dy in [(0,1), (0, -1), (1, 0), (-1, 0)]:
            xx, yy = nxt[0] + dx, nxt[1] + dy
            if (xx, yy) not in main_loop and (xx, yy) in mapping:
...
def get_anti_neighbors(mapping, main_loop, x, y):
    val = mapping[(x, y)]
    # ignore '.' 
    if val not in outer_neighbors:
        return []
    possible = outer_neighbors[val]
    n = []
    for dx, dy in possible:
        xx, yy = x + dx, y + dy
        val = mapping.get((xx, yy), '.')
        if (xx, yy) not in main_loop:
            n.append((xx, yy))
    return n
...
if __name__=='__main__':
    ...
    example_ans = solve2(example2, (1,1))
    print(f'example 2:\n {example_ans}')
    actual_ans = solve2(example3, (12, 4))
    print(f'example 3:\n {actual_ans}')

    actual_ans = solve2(actual_2, (24,79))
    print(f'actual 2:\n {actual_ans}')
