...
def check_adjacent(i, j, mapping):
    for x in range(-1, 2):
        for y in range(-1, 2):
            xi = x+i
            yj = y+j
        ...
        for i in range(len(inp[j])):
            ...
            if c.isdigit():
                if num is None:
                    ...
                    num += c
                gear = check_gear(i, j, mapping)
                if gear is not None:
                    maybe_gear = gear
            elif num is not None:
                number = int(num)
                if maybe_gear is not None:
                    gear_map[maybe_gear].append(number)
                num = None
                maybe_gear = None
            else:
                continue
